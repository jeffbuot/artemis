...
public class AddSubject extends javax.swing.JPanel {
    ...
    //add
    public AddSubject(WebPopOver pop, Connection con) {
        this.con = con;
        popOver = pop;
        isUpdate = false;
        initComponents();
    ...
    //update
    public AddSubject(WebPopOver pop, Connection con, HashMap<String, String> hs) {
        this.con = con;
        popOver = pop;
        isUpdate = true;
        initComponents();
        ...
        lblTitle.setText("Subject Data Update");
        txtID.setEditable(false);
        txtID.setText(hs.get("id"));
        txtDesc.setText(hs.get("desc"));
        int le = Integer.parseInt(hs.get("lect_units"));
        int la = Integer.parseInt(hs.get("lab_units"));
        if (le == 0) {
            optLab.setSelected(true);
            spnLab.setValue(la);
            spnLab.setEnabled(true);
            spnLect.setEnabled(false);
        } else if (la == 0) {
            optLec.setSelected(true);
            spnLect.setValue(le);
            spnLab.setEnabled(false);
            spnLect.setEnabled(true);
        } else {
    ...
    public static int showDialog(Component comp, Connection con) {
        ...
        view.setModal(true);
        view.setMargin(0);
        AddSubject a = new AddSubject(view, con);
        view.add(a);
        view.show(comp);
    ...
    public static int update(Component comp, Connection con, HashMap<String, String> hs) {
        ...
        view.setModal(true);
        view.setMargin(0);
        AddSubject a = new AddSubject(view, con, hs);
        view.add(a);
        view.show(comp);
        return a.getSubject_added();
    }
// </editor-fold> 
    ...
    //client addSubject
    public AddSubject(WebPopOver pop, BoardListUpdate blu, String ins) {
        this.blu = blu;
        popOver = pop;
        isUpdate = false;
        client = true;
    ...
    //client update
    public AddSubject(WebPopOver pop, HashMap<String, String> hs) {
        popOver = pop;
        isUpdate = true;
        client = true;
        initComponents();
        ...
        cmbProgramFacilitator.removeAllItems();
        cmbProgramFacilitator.addItem(hs.get("program_facilitator"));
        ...
        txtID.setText(hs.get("id"));
        txtDesc.setText(hs.get("desc"));
        int le = Integer.parseInt(hs.get("lec_units"));
        int la = Integer.parseInt(hs.get("lab_units"));
        if (le == 0) {
            optLab.setSelected(true);
            spnLab.setValue(la);
    ...
    public static String[] clientUpdate(Component comp, HashMap<String, String> hs) {
        ...
        view.setModal(true);
        view.setMargin(0);
        AddSubject a = new AddSubject(view, hs);
        view.add(a);
        view.show(comp);
    ...
    public static List<QueryData> clientShowDialog(Component comp, BoardListUpdate blu, String ins) {
        ...
        view.setModal(true);
        view.setMargin(0);
        AddSubject a = new AddSubject(view, blu, ins);
        view.add(a);
        view.show(comp);
    ...
    private void check(java.awt.event.ActionEvent evt) {
        ...
        int lect = spnLect.isEnabled() ? (int) spnLect.getValue() : 0;
        int lab = spnLab.isEnabled() ? (int) spnLab.getValue() : 0;
        if (id.equals("")) {
        ...
        if (client) {
            if (isUpdate) {
                String q = String.format(" update subjects set description = '%s',"
                        + "lect_units = %d,lab_units=%d,"
                        + "program_facilitator='%s' where id = '%s'",
                        desc, lect, lab, cmbProgramFacilitator.getSelectedItem(), txtID.getText());
                this.queryUpdate = new String[]{q, desc};
                popOver.dispose();
            } else {
                if (blu.getListSubjects().stream().map(s -> s.getId()).anyMatch(s -> s.equals(id))) {
                    TooltipManager.showOneTimeTooltip(txtID, null, IconFactory.infoIcon, "The subject id already exists.");
                    return;
                }

                String q = String.format("call addSubject("
                        + "'%s','%s',%d,%d,'%s')", id, desc, lect, lab,
                        cmbProgramFacilitator.getSelectedItem());
                queryData.add(new QueryData(id, "Add new subject (" + desc + ").", q));
                if (exit.isSelected()) {
                    popOver.dispose();
                } else {
                    txtID.setText("");
                    txtDesc.setText("");
                    TooltipManager.showOneTimeTooltip(btnSubmit, null, IconFactory.succIcon, "Subject added to request queue.");
                }
            }
        } else {
            try {
                if (isUpdate) {
                    ...
                    if (DBUtil.toList(con, "call showAllSubjects", "id").stream().anyMatch(i -> i.equals(id))) {
                        TooltipManager.showOneTimeTooltip(txtID, null, IconFactory.infoIcon, "The subject id already exists.");
                        return;
                    }
                    con.createStatement().executeUpdate(String.format("call addSubject("
                            + "'%s','%s',%d,%d,'%s')", id, desc, lect, lab,
                            cmbProgramFacilitator.getSelectedItem()));
                    subject_added++;
                    if (exit.isSelected()) {
                        ...
                        txtID.setText("");
                        txtDesc.setText("");
                        TooltipManager.showOneTimeTooltip(btnSubmit, null, IconFactory.succIcon, "Subject successfully added.");
                    }
                }
            } catch (SQLException e) {
    ...
    private void initComponents() {
        ...
        spnLect.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        spnLect.setModel(new javax.swing.SpinnerNumberModel(Integer.valueOf(3), null, Integer.valueOf(10), Integer.valueOf(1)));
        ...
        jLabel8.setText("Program Sched:");

        groupClassSpecification.add(optLec);
        optLec.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        optLec.setSelected(true);
        optLec.setText("Lecture Only");
        optLec.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                optLecActionPerformed(evt);
            }
        });

        groupClassSpecification.add(optLab);
        optLab.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        optLab.setText("Lab Only");
        optLab.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                optLabActionPerformed(evt);
            }
        });

        groupClassSpecification.add(optBoth);
        optBoth.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        optBoth.setText("Lecture & Lab");
        optBoth.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                optBothActionPerformed(evt);
            }
        });

        lblLect.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lblLect.setText("Lect:");

        lblLab.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        lblLab.setText("Lab:");

        spnLab.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        spnLab.setModel(new javax.swing.SpinnerNumberModel(Integer.valueOf(3), null, Integer.valueOf(10), Integer.valueOf(1)));
        spnLab.setEnabled(false);
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(layout.createSequentialGroup()
                                ...
                                .addComponent(optLab)
                                .addGap(18, 18, 18)
                                .addComponent(optBoth, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(lblLect)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(spnLect, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(18, 18, 18)
                                .addComponent(lblLab)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(spnLab, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    ...
                    .addComponent(optLec)
                    .addComponent(optLab)
                    .addComponent(optBoth))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabel5, javax.swing.GroupLayout.PREFERRED_SIZE, 30, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(spnLect, javax.swing.GroupLayout.PREFERRED_SIZE, 29, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(lblLect, javax.swing.GroupLayout.PREFERRED_SIZE, 30, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(spnLab, javax.swing.GroupLayout.PREFERRED_SIZE, 29, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(lblLab, javax.swing.GroupLayout.PREFERRED_SIZE, 30, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
    ...
    private void optLecActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_optLecActionPerformed
        lblLect.setEnabled(true);
        spnLect.setEnabled(true);
        lblLab.setEnabled(false);
        spnLab.setEnabled(false);
    }//GEN-LAST:event_optLecActionPerformed

    private void optLabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_optLabActionPerformed
        lblLect.setEnabled(false);
        spnLect.setEnabled(false);
        lblLab.setEnabled(true);
        spnLab.setEnabled(true);
    }//GEN-LAST:event_optLabActionPerformed

    private void optBothActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_optBothActionPerformed
        lblLect.setEnabled(true);
        spnLect.setEnabled(true);
        lblLab.setEnabled(true);
        spnLab.setEnabled(true);
    }//GEN-LAST:event_optBothActionPerformed
    ...
    private javax.swing.JRadioButton optBoth;
    private javax.swing.JRadioButton optLab;
    private javax.swing.JRadioButton optLec;
    private javax.swing.JSpinner spnLab;
    private javax.swing.JSpinner spnLect;
